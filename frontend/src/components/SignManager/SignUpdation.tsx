...
interface SignUpdationProps {
    updateId: string;
    toggleUpdation: () => void;
}

const SignUpdation: React.FC<SignUpdationProps> = ({ updateId, toggleUpdation }) => {
    interface Sign {
        id: string;
        code: string;
        title: string;
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (sign.imageName != null) {
            formData.append("imageName", sign.imageName);
        }
        ...
        try {
            await signApi.updateSign(formData);
        } catch (error: any) {
    ...
    useEffect(() => {
        ...
        const fetchApi = async () => {
            try {
                const { data } = await signApi.getSignById(updateId);
                setSign(data);
            } catch (error) {
                console.error("Lỗi khi gọi API sign:", error);
            }
        };
        fetchApi();
    }, [updateId]);
        ...
        <div className="relative p-4 w-full max-w-2xl max-h-full">
            <div className="relative p-4 bg-white rounded-sm shadow sm:p-5">
                <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Cập nhật biển báo</h3>
