...
interface NewsUpdationProps {
    updateId: string;
    toggleUpdation: () => void;
}

const NewsUpdation: React.FC<NewsUpdationProps> = ({ updateId, toggleUpdation }) => {
    ...
    useEffect(() => {
        ...
        const fetchApi = async () => {
            try {
                const { data } = await newsApi.getUpdatedNews(updateId);
                setNews(data);
            } catch (error) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (news.thumbnailName != null) {
            formData.append("thumbnailName", news.thumbnailName);
        }
